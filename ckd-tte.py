...
logging.basicConfig(
    ...
    datefmt='%H:%M:%S',
    handlers=[
        logging.FileHandler("training_e2e2.log", mode='w'),
...
def parse_args():
    ...
    parser.add_argument("--num-layers", type=int, default=4, help="Layers for RNN/LSTM/Transformer/TCN/ODE MLP block.")
...
def build_sequences_for_multitask(meta, window_size):
    ...
    for pid, group in meta.groupby("PatientID"):
        ...
        for i in range(1, len(embeddings)):
            ...
            target = labels[i - 1]
            # For time to event, use the value from the row whose label we are predicting
            # If that row has time_until_progression as T, we train the model to predict T
...
def train_and_evaluate(model, device, train_loader, val_loader, test_loader, args, model_name):
    ...
    if isinstance(model, MultiTaskTimeToEventRNN):
        # classification + MSE on time
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            if isinstance(model, MultiTaskTimeToEventRNN):
                ...
                # Only apply MSE if there's a real TTE label (NaN can be masked)
...
def main():
    ...
    logger.info("Optional step: adding time-to-event columns for multi-task learning.")
    # If you only want classification, skip this. Otherwise, you can do:
    ...
    # Original approach: classification-only
    ...
    logger.info("Defining our original classification models.")
    ...
    logger.info("Defining a multi-task model: classification + time-to-event.")
    ...
    logger.info("Training the classification-only models (RNN, LSTM, Transformer, MLP, TCN, ODE).")
    ...
    if TORCHDIFFEQ_AVAILABLE:
        ode_results = train_and_evaluate(ode_model, device, train_loader_class, val_loader_class, test_loader_class, args, "NeuralODE")
    else:
        logger.info("torchdiffeq not installed, skipping Neural ODE training.")
        ode_results = None

    logger.info("Training multi-task RNN (classification + time-to-event).")
    multitask_results = train_and_evaluate(multitask_rnn, device, train_loader_tte, val_loader_tte, test_loader_tte, args, "MultiTaskRNN")
    ...
    if ode_results is not None:
        models_for_analysis.append(("NeuralODE", ode_model))
    models_for_analysis.append(("MultiTaskRNN", multitask_rnn))

    for name, model_obj in models_for_analysis:
        df_preds = predict_label_switches(model_obj, test_loader_class, device) if name != "MultiTaskRNN" else predict_label_switches(model_obj, test_loader_tte, device)
