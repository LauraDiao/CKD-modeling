...
batch_size_ = 1024*len(device_ids)
n_workers = 16*len(device_ids) # 0, 12, 16, 20
...
def train_and_evaluate_deepsurv(model, device, train_loader, val_loader, test_loader, args, model_name):
    ...
    if torch.cuda.device_count() > 1:
        ...
        model = nn.DataParallel(model, device_ids=visible_indices)
...
def train_and_evaluate(model, device, train_loader, val_loader, test_loader, args, model_name):
    ...
    if torch.cuda.device_count() > 1:
        logger.info(f"Using {torch.cuda.device_count()} GPUs")
        model = nn.DataParallel(model, device_ids=visible_indices) # verify ids
...
logger.info(f"Using device: cuda:{os_ids}")
