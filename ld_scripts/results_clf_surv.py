...
def bootstrap_metrics_gpu(y_true, y_probs, threshold, n_iterations=1000, gpu_id=3):
    ...
    # 1. Move 1.45M rows to VRAM (once per model)
    ...
    boot_results = {k: [] for k in ["AUROC", "AUPRC", "F1", "PPV", "Recall"]}
    ...
    for b in range(0, n_iterations, batch_size):
        ...
        for i in range(current_batch_size):
            ...
            boot_results["AUROC"].append(roc_auc_score(y_t_cpu, y_p_cpu))
            boot_results["AUPRC"].append(average_precision_score(y_t_cpu, y_p_cpu))
    ...
    for k, v in boot_results.items():
        ...
        # ADD THIS: Pre-formatted string for your DataFrame rows
        # Format: "0.850 (0.840-0.860)"
        final_stats[f"{k}_formatted"] = f"{mean:.3f} ({low:.3f}-{high:.3f})"
...
def evaluate_models(filepaths, n_boot=1000, n_workers=8, threshold_cap=1.0, threshold_step=0.01, verbose=False):
    ...
    for filepath in filepaths:
        ...
        boot_metrics = bootstrap_metrics(y_true, y_probs, threshold, n_iterations=n_boot, n_workers=n_workers)
        # boot_metrics = bootstrap_metrics_gpu(y_true, y_probs, threshold, n_iterations=n_boot)
        if verbose:
            print("Bootstrapped Metrics with 95% Confidence Intervals:")
            for k, (mean, low, high) in boot_metrics.items():
                print(f"  {k}: {mean:.4f} [{low:.4f}, {high:.4f}]")
...
# %%
# baseline
# fp = f"./365day_future_prediction_outputs_stage_filter_full_stage_filter_eskd_v2"

# baseline patient level
# fp = f"./365day_future_prediction_outputs_stage_filter_full_stage_filter_eskd_v2_patient_level"

# dirs = ['/XGBoost_365DayFuture_Classifier_detailed_outputs_classification.csv']

# dirs = ['/XGBoost_365DayFuture_Classifier_detailed_outputs_classification_pt_lvl.csv']
...
with open(log_path, 'w') as f:
    with redirect_stdout(f):
        ...
        out_path = os.path.join("results_files_v2", fp.strip("./") + f"_{modifier}_metrics.csv")
        out_path
