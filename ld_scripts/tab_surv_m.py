    ...
    def __getitem__(self, idx):
        if self.use_survival_multitask:
            ...
            return (
                ...
                torch.tensor(classification_target, dtype=torch.long), # Classification target (e.g. label_ckd_{years}_year_future)
                pid,
                local_idx,
...
def build_sequences_for_multitask(meta, window_size, 
                                  classification_target_col=f'label_ckd_{years}_year_future',
...
def train_and_evaluate_deepsurv(model, device, train_loader, val_loader, test_loader, args, model_name):
    logger.info(f"Starting {model_name} training (DeepSurv multi-task). Classification target: 'label_ckd_{years}_year_future'.")
    model.to(device)
    ...
    # Classification metrics (based on label_ckd_{years}_year_future)
    ...
    if len(all_classification_targets_test) > 0:
        ...
        logger.info(f"{model_name} Classification Test Prevalence ('label_ckd_{years}_year_future'): {prevalence:.4f}")
    ...
    # Event for C-index is all_classification_targets_test (label_ckd_{years}_year_future)
    ...
    if np.sum(valid_cindex_mask) > 1 and np.sum(all_classification_targets_test[valid_cindex_mask]) > 0 : # Need at least one event
        c_index_val = concordance_index(
            ...
            event_observed_np=all_classification_targets_test[valid_cindex_mask] # label_ckd_{years}_year_future as event
        )
        logger.info(f"{model_name} Concordance Index (Time: time_until_progression, Event: label_ckd_{years}_year_future, Risk: model_risk_head): {c_index_val:.4f}")
...
def train_and_evaluate(model, device, train_loader, val_loader, test_loader, args, model_name):
    logger.info(f"Starting {model_name} training (Classification Target: 'label_ckd_{years}_year_future').")
    model.to(device)
    classification_criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=args.scheduler_patience, verbose=False)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader: # Standard classification loader
            optimizer.zero_grad()
            x_batch, y_batch, _, _ = batch # y_batch is label_ckd_{years}_year_future
            x_batch = x_batch.to(device)
...
# They will use the new `label_ckd_{years}_year_future` as the "TrueLabel" implicitly via the dataloaders
...
true
    ...
    logger.info(f"Building sequence datasets for classification (Target: 'label_ckd_{years}_year_future').")
    ...
    # Summarize Results
    logger.info(f"\n--- Summary: Final Test Metrics (Classification Models - Tabular, Target: 'label_ckd_{years}_year_future') ---")
    ...
    logger.info(f"\n--- Summary: Final Test Metrics (DeepSurv Multi-Task Models - Tabular, Target: 'label_ckd_{years}_year_future') ---")
